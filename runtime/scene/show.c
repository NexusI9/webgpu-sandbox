...
void scene_show_mesh(Scene *scene, Mesh *mesh, const ScenePipeline pipeline) {
  ...
  // prevent duplicate
  if (mesh_reference_list_find(&scene->pipelines[pipeline], mesh) != NULL)
    return;
...
void scene_hide_mesh(Scene *scene, Mesh *mesh, const ScenePipeline pipeline) {
  mesh_reference_list_remove(&scene->pipelines[pipeline], mesh);
}

void scene_show_mesh_reference_list(Scene *scene, MeshRefList *list,
                                    const ScenePipeline pipeline) {
  for (size_t i = 0; i < list->length; i++)
    scene_show_mesh(scene, list->entries[i], pipeline);
}

void scene_hide_mesh_reference_list(Scene *scene, MeshRefList *list,
                                    const ScenePipeline pipeline) {
  for (size_t i = 0; i < list->length; i++)
