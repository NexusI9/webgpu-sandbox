...
static void raycast_camera_screen(Raycast *, Camera *, Viewport *, float,
                                  float);

void raycast_camera_screen(Raycast *ray, Camera *cam, Viewport *vp, float x,
                           float y) {
...
void raycast_camera_cursor(Raycast *ray, Camera *cam, Viewport *vp) {

  // convert screen to NDC
  float x = (2.0f * g_input.mouse.x) / vp->width - 1.0f;
  float y = 1.0f - (2.0f * g_input.mouse.y) / vp->height;

  raycast_camera_screen(ray, cam, vp, x, y);
}

void raycast_camera_center(Raycast *ray, Camera *cam, Viewport *vp) {

  // convert screen to NDC
  raycast_camera_screen(ray, cam, vp, 0.0f, 0.0f);
}

Mesh *raycast_check_mesh_list(Raycast *ray, const MeshRefList *list) {
