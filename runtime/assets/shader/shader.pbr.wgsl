...
fn point_shadow_position_f(world_position : vec3<f32>, light_view : mat4x4<f32>)
    -> f32 {
  ...
  return shadow_clip.w; // / shadow_clip.w;
}
...
@fragment fn fs_main(@location(0) vNormal : vec3<f32>,
  ...
  let shadow_factor = point_shadow_factor(vFrag);
  // let shadow_position =
  //     point_shadow_position(vFrag, point_light_list.items[0].views[0]);

  // return vec4<f32>(color * shadow_color, 1.0f);

  return vec4<f32>(shadow_factor, 0.0f, 0.0f, 1.0f);
  // return vec4<f32>(vFrag, 1.0f);
