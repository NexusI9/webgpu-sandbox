...
ShaderBindGroupUniformEntry *shader_find_uniform(Shader *shader,
  ...
  for (size_t i = 0; i < bind_group->uniforms.length; i++)
    if (bind_group->uniforms.entries[i].binding == index)
      return &bind_group->uniforms.entries[i];
...
ShaderBindGroupTextureEntry *shader_find_texture(Shader *shader,
  ...
  if (bind_group == NULL) {
    VERBOSE_WARNING("Could not find the group index at %d (shader: %s)",
                    group_index, shader->name);
    return NULL;
  } else if (bind_group->textures.entries == NULL) {
    VERBOSE_WARNING("Textures not initialized in bind group: %d (shader: %s)",
                    group_index, shader->name);
    return NULL;
  }

  for (size_t i = 0; i < bind_group->textures.length; i++)
    if (bind_group->textures.entries[i].binding == index)
      return &bind_group->textures.entries[i];
...
ShaderBindGroupSamplerEntry *shader_find_sampler(Shader *shader,
  ...
  if (bind_group == NULL) {
    VERBOSE_WARNING("Could not find the group index at %d (Shader: %s)",
                    group_index, shader->name);
    return NULL;
  } else if (bind_group->samplers.entries == NULL) {
    VERBOSE_WARNING("Samplers not initialized in bind group: %d (shader: %s)",
                    group_index, shader->name);
    return NULL;
  }

  for (size_t i = 0; i < bind_group->samplers.length; i++)
    if (bind_group->samplers.entries[i].binding == index)
      return &bind_group->samplers.entries[i];
