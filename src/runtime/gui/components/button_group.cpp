...
bool UI::ButtonGroup::draw() {
  ...
  for (int i = 0; i < items_count; i++) {
    ...
    const ThemeIconCell *uv = theme_icon_cell(gui->theme, items[i].icon);
    draw_list->AddImage((ImTextureRef)theme_icon_atlas(gui->theme), imgMin,
                        imgMax, ImVec2(uv->uv0[0], uv->uv0[1]),
