...
static inline void compute_pass_bgra2rgba_cache_resources(ComputePass *);

BGRA2RGBAStatus
compute_pass_bgra2rgba_create(ComputePass *pass,
  ...
  if (layer_count > 2 * COMPUTE_PASS_VIEW_CAPACITY) {
    ...
    return BGRA2RGBAStatus_OutOfBound;
  }

  compute_pass_create(pass, &(ComputePassDescriptor){
                                .label = desc->label,
                                .source_texture = desc->source_texture,
                                .buffer_texture = desc->buffer_texture,
                            });

  compute_pass_bgra2rgba_update_source_texture(pass, desc->source_texture);

  return BGRA2RGBAStatus_Success;
}

BGRA2RGBAStatus compute_pass_bgra2rgba_draw(ComputePass *pass) {

  WGPUCommandEncoder command_encoder =
      wgpuDeviceCreateCommandEncoder(context_device(), NULL);

  compute_pass_bgra2rgba_draw_inline(pass, command_encoder);
...
BGRA2RGBAStatus
compute_pass_bgra2rgba_update_source_texture(ComputePass *pass,
  ...
  // rebuild bindgroups
  compute_pass_bgra2rgba_cache_resources(pass);

  return BGRA2RGBAStatus_Success;
}

void compute_pass_bgra2rgba_cache_resources(ComputePass *pass) {

  const ComputePipeline *compute_pipeline =
      std_compute_pipeline(ComputePipelineType_BGRA2RGBA);
  ...
  for (uint32_t i = 0; i < pass->layer_count; i++) {
    ...
    b_desc.format = wgpuTextureGetFormat(pass->buffer_texture);
    b_desc.baseArrayLayer = 0; // reset array layer
    ...
    static const int entries_count = sizeof(entries) / sizeof(entries[0]);
...
BGRA2RGBAStatus
compute_pass_bgra2rgba_draw_inline(ComputePass *pass,
  ...
  for (uint32_t i = 0; i < pass->layer_count; i++) {
    ...
    WGPUComputePassEncoder compute_pass = wgpuCommandEncoderBeginComputePass(
        command_encoder,
        &(WGPUComputePassDescriptor){.label = "BGRA2RGBA Compute Pass"});
    ...
    compute_pass_dispatch(compute_pass, BGRA2RGBA_WORKGROUP, pass->width,
                          pass->height);
