...
fn vs_main(input: VertexIn) -> VertexOut {
  ...
  // Put the grid below the camera
    var translate_matrix: mat4x4<f32> = mat4x4<f32>(vec4<f32>(1.0, 0.0, 0.0, 0.0),
        vec4<f32>(0.0, 1.0, 0.0, 0.0),
        vec4<f32>(0.0, 0.0, 1.0, 0.0),
        vec4<f32>(offset.x, 0.0, offset.y, 1.0));

    output.Position = cam * translate_matrix * mesh.model * vec4<f32>(input.aPos, 1.0);
...
fn fs_main(@location(0) vCol: vec3<f32>,
  ...
  // Move Uv to the opposite camera direction to compensate the grid translation
    ...
    var gridUv: vec2<f32> = sign(vec2(edge) - fract(compensUv / patternSize) * patternSize);

    var pattern: vec4<f32> = vec4(face_tone - sign(gridUv.x + gridUv.y + 1.0) * (face_tone - edge_tone));
  ...
  // Add X & Y Axis
    ...
    var yAxis: f32 = step(abs(vUv.x - 0.5f + offset.x / uGrid.scale), axisThickness);
    var xAxis: f32 = step(abs(vUv.y - 0.5f + offset.y / uGrid.scale), axisThickness);
  ...
  // Setup gradient
    ...
    return (uGrid.color * pattern * axisMask + vec4(axis, 1.0f)) * vec4(grad, avg);
}
