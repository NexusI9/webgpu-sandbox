                                          ...
                                          views : array<mat4x4<f32>, 6>,
                                                  inner_cutoff : f32,
                                                                 near : f32,
                                                                        far
      : f32,
        _padding : array<f32, 21>,
};

struct AmbientLight {
  color : vec3<f32>, intensity : f32, _padding : array<f32, 60>,
};
...
@group(2) @binding(0) var<storage, read> ambient_light_list
    : array<AmbientLight>;
@group(2) @binding(1) var<storage, read> spot_light_list : array<SpotLight>;
@group(2) @binding(2) var<storage, read> point_light_list : array<PointLight>;
@group(2) @binding(3) var<storage, read> sun_light_list : array<SunLight>;
@group(2) @binding(4) var<uniform> ubo : UBO;


@group(2) @binding(5) var point_shadow_maps : texture_depth_cube_array;
@group(2) @binding(6) var point_shadow_sampler : sampler_comparison;
@group(2) @binding(7) var directional_shadow_maps : texture_depth_2d_array;
@group(2) @binding(8) var directional_shadow_sampler : sampler_comparison;
...
fn point_shadow_factor(frag_position : vec3<f32>, cam_pos : vec3<f32>) -> f32 {
  ...
  for (var l : u32 = 0u; l < ubo.light_count.point; l++) {

    let light = point_light_list[l];
...
fn spot_shadow_factor(frag_position : vec3<f32>, cam_pos : vec3<f32>) -> f32 {
  ...
  for (var l : u32 = 0u; l < ubo.light_count.spot; l++) {

    let light = spot_light_list[l];
    let shadow_position =
...
fn sun_shadow_factor(frag_position : vec3<f32>, cam_pos : vec3<f32>) -> f32 {
  ...
  for (var l : u32 = 0u; l < ubo.light_count.sun; l++) {

    let layer = ubo.light_count.spot + l;
    let light = sun_light_list[l];
    let shadow_position =
        directional_shadow_position(frag_position, light.view);
...
@fragment fn fs_main(@location(0) vNormal : vec3<f32>,
  ...
  for (var i = 0u; i < ubo.light_count.point; i++) {
    let light = point_light_list[i];
    Lo += compute_point_light(material, vFrag, vNormal, camera.position.xyz,
  ...
  for (var i = 0u; i < ubo.light_count.spot; i = i + 1u) {
    let light = spot_light_list[i];
  ...
  for (var i = 0u; i < ubo.light_count.sun; i = i + 1u) {
    let light = sun_light_list[i];
    Lo += compute_point_light(material, vFrag, vNormal, camera.position.xyz,
                              light.position, light.color, light.intensity);
  }
  ...
  for (var i = 0u; i < ubo.light_count.ambient; i = i + 1u) {
    let light = ambient_light_list[i];
